...
const vcard = 'BEGIN:VCARD\n' //
            + 'VERSION:3.0\n' //
            + 'FN: OWNER SI GANTENG\n' //
            + 'ORG:Creator vr.zaq BOT;\n' //
            + 'TEL;type=CELL;type=VOICE;waid=62895622957611:+62895-6229-57611\n' // NOMER HP LU
            + 'END:VCARD'
prefix = '!'
blocked = ['62895622957611@s.whatsapp.net']
...
async function starts() {
	...
	client.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `[ NEW MEMBER IN GROUP _*${mdata.subject}*_ ] \n*_____________*\n@${num.split('@')[0]}\nSELAMAT BERGABUNG\n *_____________*\nMoga betah Di group Dan Jangan Lupa Intro Yha Dan Jangan Lupa subscribe Arifi Razzaq OFFICIAL yha Heh!`
				...
				teks = `hey kak @${num.split('@')[0]} vr.zaq BOT Titip Gorengan Yah Btw Gausa Kembali Lagi... Kembali = kick 😋`
				let buffer = await getBuffer(ppimg)
				client.sendMessage(mdata.id, buffer, MessageType.image, {caption: teks, contextInfo: {"mentionedJid": [num]}})
			}
		} catch (e) {
	...
	client.on('message-new', async (mek) => {
		try {
			...
			mess = {
				wait: '[TUNGGU] vr.zaq BOT sedang Memperoses🍥Orang sabar Di Sayang Dajal!',
				asik: '[TUNGGU] vr.zaq BOT sedang Memperoses🍥Orang sabar Di Sayang Dajal!',
				...
				only: {
					...
					mod: '[❗] PERINTAH INI KHUSUS USER *MOD vr.zaq BOT*',
					benned: 'Anda Ke Band Silahkan Hubungi Owner Agar Membuka Band Anda',
					ownerG: '[❗] *Woy Kau Siapa! Owner? ini fitur khusus owner Kamprettt! Tolol Banget Sih Lu Jadi Human :v* ❌',
					ownerB: '[❗] *Woy Kau Siapa! Owner? ini fitur khusus owner Kamprettt! Tolol Banget Sih Lu Jadi Human :v* ❌',
					userB: `──「 DAFTAR 」──\nHalo Kak !\nMaaf Kak Kamu belum Terdaftar Sebagai Teman vr.zaq BOT Daftar Untuk Berteman Dengan Ici bot dengam cara, \n\nCommand : ${prefix}daftar nama|umur\nContoh : ${prefix}daftar vr.zaq BOT|17\n\n──「 vr.zaq BOT 」──`,
			...
			const mod = [ownerNumber,"62895622957611@s.whatsapp.net"]//ubah nomor lo
			const adminbotnumber = ["62895622957611@s.whatsapp.net"]//ubah nomor lo
			const frendsowner = ["62895622957611@s.whatsapp.net"]//ubah nomor lo
			const premium = ["62895622957611@s.whatsapp.net","6283191156893@s.whatsapp.net"]
       ...
       //function leveling
			...
			switch(command) {
				...
				if (!isUser) return reply(mess.only.userB)
					...
					break
					case 'vr.zaq BOTgroup':
					if (isBanned) return reply(mess.only.benned)    
					if (!isUser) return reply(mess.only.userB)
					client.sendMessage(from, '*vr.zaq BOT GROUP*\n\nLink : https://chat.whatsapp.com/E6HNR5o4r6n448IptYTgOR*', text, { quoted: mek })
					break
					case 'funmenu':
					if (isBanned) return reply(mess.only.benned)    
					if (!isUser) return reply(mess.only.userB)
			       ...
			       case 'valxlove':
					...
					break
					case 'vr.zaq BOTadmin':
					if (isBanned) return reply(mess.only.benned)    
					if (!isUser) return reply(mess.only.userB)
					tod = await getBuffer(`https://i.ibb.co/XDwBVDJ/1f2652c622fa.jpg`)
					client.sendMessage(from, tod, image, { quoted: mek, caption: '*╭────*「 *ADMINBOT vr.zaq BOT ✨* 」\n*│+wa.me/62895622957611*╰──────*「 *vr.zaq BOT* 」*────*\n\n*_JIKA INGIN MENJADI ADMIN  vr.zaq BOT_*\n*_Ketik /iklan_*' })
					break
					case 'premiumcek':
					if (isBanned) return reply(mess.only.benned)    
					if (!isUser) return reply(mess.only.userB)
					sa = await getBuffer(`https://i.ibb.co/PcQ6tsB/79ac87b9358c.jpg`)
					client.sendMessage(from, sa, image, { quoted: mek, caption: '*╭────*「 *PREMIUM USER👑* 」\n*│+ wa.me/62895622957611*╰──────*「 * vr.zaq BOT* 」*────*\n\n*_JIKA INGIN MENJADI PREMIUM USER  vr.zaq BOT_*\n*_Ketik #daftarvip*' })
					break
					case 'cekmod': 
           ...
           if (!isUser) return reply(mess.only.userB)
            ...
            break
				...
				mentioned = mek.message.extendedTextMessage.contextInfo.mentionedJid
					...
					break
                case 'fakereplay':
                   client2.fakeReply("ange mas", "mending lari", "62895622957611", MessageType.text)
                   break
				case 'infogc':
				if (isBanned) return reply(mess.only.benned)    
				if (!isUser) return reply(mess.only.userB)
				...
				if (!isUser) return reply(mess.only.userB)
					...
					if (args.length < 1) return reply(`Parameter Salah\nCommand : ${prefix}daftar nama|umur\nContoh : ${prefix}daftar vr.zaq BOT|12`)
				...
				if (!isUser) return reply(mess.only.userB)
					if (!isOwner) return reply(mess.only.ownerB)    
					teks = `\`\`\`╭────*「 *TOTAL USER BOT  vr.zaq BOT👑* 」\n\`\`\``
					no = 0
...
if (isBanned) return reply(mess.only.benned)    
               ...
               client.reply(from, `Waalaikumsalam wr.wb${pushname}:)`)
				...
				if (isBanned) return reply(mess.only.benned)    
				if (!isUser) return reply(mess.only.userB)
					client.sendMessage(from, '*Hai kamu!* <br/> *Tolong bantuan donasi-nya Agar Bot Selalu Aktif 😘* <br/> *Via DANA :* _0822-7410-2032_\n <br/> *Via SAWERIA :* _https://saweria.co/Arifirazzaq2001\n <br/> *Via GOPAY :* _0822-7410-2032_', text, { quoted: mek })
					break
                case 'ttp':
                if (isBanned) return reply(mess.only.benned)    
				if (!isUser) return reply(mess.only.userB)
  ...
  client.updatePresence(from, Presence.composing) 
    ...
    client.sendMessage(from, options, text, { quoted: mek } )
		...
		if (args.length < 1) return reply('tag temanmu!')
				...
				reply(ez)
				break
				case 'vr.zaq BOTjokes':
				client.updatePresence(from, Presence.composing) 
 if (isBanned) return reply(mess.only.benned)    
				if (!isUser) return reply(mess.only.userB)
				 data = fs.readFileSync('./src/vr.zaq BOTjokes.js');
                 jsonData = JSON.parse(data);
                 randIndex = Math.floor(Math.random() * jsonData.length);
                 randKey = jsonData[randIndex];
                hasil = await getBuffer(randKey.result)
                client.sendMessage(hasil, image, mek, '\`\`\`vr.zaq BOTJOKES\`\`\`')
				break
					case 'kata':
 ...
 if (!isOwner) return reply(mess.only.ownerB)
			...
			if (isBanned) return reply(mess.only.benned)    
				if (!isUser) return reply(mess.only.userB)
					...
					if (!isGroup) return reply(mess.only.group)
					if (!isOwner) return reply('[❗] *Woy Kau Siapa! Owner? ini fitur khusus owner Kamprettt! Tolol Banget Sih Lu Jadi Human :v* ❌')
					members_id = []
					teks = (args.length > 1) ? body.slice(8).trim() : ''
					teks += '\n\n'
					for (let mem of groupMembers) {
			...
			if (isBanned) return reply(mess.only.benned)    
				if (!isUser) return reply(mess.only.userB)
					...
					mentions('╔══✪〘 Mention All 〙✪══'+teks+'╚═〘  vr.zaq BOT 〙', members_id, true)
					break
				case 'clearall':
				if (isBanned) return reply(mess.only.benned)    
				if (!isUser) return reply(mess.only.userB)
				...
				if (!isUser) return reply(mess.only.userB)
					...
					if (isMedia && !mek.message.videoMessage || isQuotedImage) {
						...
						for (let _ of anu) {
							sendMess(_.jid, `[ *vr.zaq BOT Broadcast* ]\n\n${body.slice(4)}`)
						}
						reply('Suksess broadcast')
					}
					break
		...
		if (isBanned) return reply(mess.only.benned)    
				if (!isUser) return reply(mess.only.userB)
                    ...
                    if (!isGroup) return reply(mess.only.group)
                    if (!isadminbot) return reply('[❗] *Woy Kau Siapa! Owner? ini fitur khusus owner Kamprettt! Tolol Banget Sih Lu Jadi Human :v* ❌')
                    if (!isBotGroupAdmins) return reply(mess.only.Badmin)
                    media = await client.downloadAndSaveMediaMessage(mek)
                    await client.updateProfilePicture (from, media)
                    reply('Sukses mengganti icon Grup')
	...
	.then(() => {client.sendMessage(from, options, text)})
				...
				if (!isUser) return reply(mess.only.userB)
                    ...
                    if (!isGroup) return reply(mess.only.group)
                    if (!isfrendsowner) return reply('[❗] *Woy Kau Siapa! Owner? ini fitur khusus owner Kamprettt! Tolol Banget Sih Lu Jadi Human :v* ❌')
                    if (!isBotGroupAdmins) return reply(mess.only.Badmin)
                    media = await client.downloadAndSaveMediaMessage(mek)
                    await client.updateProfilePicture (from, media)
                    reply('Sukses mengganti icon Grup')
				...
				//frendowner
				case `addadmin`:
                   if (!isOwner) return reply('Perintah ini hanya bisa di gunakan oleh Owner vr.zaq BOT!', id)
                   for (let i = 0; i < mentionedJidList.length; i++) {
                   adminNumber.push(mentionedJidList[i])
                   fs.writeFileSync('./lib/database/admin.json', JSON.stringify(adminNumber))
                   reply('Success Menambahkan Admin vr.zaq BOT!')
				   }
                   break
				case 'wait':
				if (isBanned) return reply(mess.only.benned)    
